...
def check_perceptron(tracker):
    ...
    with no_graphics():
        ...
        for _ in range(10):
            ...
            assert pred == 1, "Untrained perceptron should predict 1 instead of {} for {}.".format(pred, pred)
    ...
    if stats["accuracy"] < 1.0:
        print("The weights learned by your perceptron correctly classified {:.2%} of training examples".format(stats['accuracy']))
...
def sanity_test_node(cls, *inputs):
    ...
    for i, el in enumerate(output_grad):
        assert isinstance(el, np.ndarray), "The gradient from {}.backward with respect to input {} is not a numpy array".format(
            name, i)
...
def check_graph_accumulator(tracker):
    ...
    assert all(np.allclose(a, b) for a, b in zip(student, expected)),\
        "Graph.get_inputs returned incorrect value for an Add node"
    ...
    for node in [v1, adder]:
        ...
        assert np.shape(node_grad) == output_shape,\
            "Graph.get_gradient returned gradient of wrong shape"
        assert np.allclose(node_grad, np.zeros_like(node_grad)),\
            "Graph.get_gradient should return all-zero values before backprop is called"

    expected_loss = 1.60943791243
    graph.backprop()
    v1_grad = graph.get_gradient(v1)
    assert np.allclose(v1_grad, np.ones_like(v1_grad) * expected_loss * 2),\
        "Incorrect gradient after running Graph.backprop(). Make sure you are correctly accumulating your gradients."
    tracker.add_points(3)
...
def check_regression(tracker):
    ...
    if stats['loss'] <= loss_threshold:
        ...
        print("Your final loss ({:f}) must be no more than {:.4f} to receive points for this question".format(stats['loss'], loss_threshold))
...
def check_digit_classification(tracker):
    ...
    if stats['dev_accuracy'] >= accuracy_threshold:
        ...
        print("Your final validation accuracy ({:%}) must be at least {:.0%} to receive points for this question".format(stats['dev_accuracy'], accuracy_threshold))
...
def check_rl(tracker):
    ...
    print("To receive credit for this question, your agent must receive a mean reward of at least {} on {} out of {} trials".format(
